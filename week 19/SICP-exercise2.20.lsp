...
; parameter list that has a dot before the last parameter name indicates that, when the procedure
; is called, the initial parameters (if any) will have as values the initial arguments, as usual,
; but the final parameter’s value will be a list of any remaining arguments. For instance, given
; the definition
...
(define (same-parity a . z)
  (define (parity-iter existing-list new-list)
    (if (null? existing-list)
      new-list
      (parity-iter (cdr existing-list)
          (if (= (remainder (car existing-list) 2)
                 (remainder a 2))
              (append new-list (list (car existing-list)))
              new-list))))
    (parity-iter z (list a)))
