...
;; using the expmod procedure. However, whenever we perform the squaring step in
...
(define (non-trivial-square base n m)
   (let ((x (remainder
          (square (expmod base (/ n 2) m)) m )))
     (cond ((and
             (= x 1)
             (= x (- m 1))
             (= (square x) (remainder 1 m))
             ) 0)
           (else x)))
           )
